...
@file:Suppress(
    ...
    "NON_EXHAUSTIVE_WHEN", "JoinDeclarationAndAssignment", "FunctionName"
)
...
class scoreboard : uvm_subscriber<result_transaction> {
    ...
    fun predict_result(cmd: command_transaction): result_transaction {
        ...
        when (cmd.op) {
            operation_t.add_op -> predicted.result = (cmd.A add cmd.B).ext()
            operation_t.and_op -> predicted.result = (cmd.A and cmd.B).ext()
            operation_t.xor_op -> predicted.result = (cmd.A xor cmd.B).ext()
            operation_t.mul_op -> predicted.result = cmd.A mul cmd.B
        }
        return predicted
    }
